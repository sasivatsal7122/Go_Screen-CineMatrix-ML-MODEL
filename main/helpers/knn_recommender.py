...
# reading the dataset
df_movies  = pd.read_csv('main/data/movies.csv')
...
movie_user_mat_sparse = scipy.sparse.load_npz('main/data/movie_user_mat_sparse.npz')
with open('main/data/movie_to_idx.json') as json_file:
...
def recommend(model_knn=model_knn,data=movie_user_mat_sparse,fav_movie=" ",mapper=movie_to_idx,n_recommendations=10):
    ...
    # set movie_ls_finale to movie_ls
    movie_ls_finale = movie_ls
    return movie_ls   
...
def KNN_recommend(user_fav_movie):
    ...
    print("Started KNN with movie: ",user_fav_movie)
    ...
    if bool(close_match):
        ...
        movie_dict = {"movie_ls_finale": movie_ls_finale, "titles_id": movie_id_ls}

        return json.dumps(movie_dict)
    else:
        print("Could'nt find movie in our database")
        movie_dict = {"movie_ls_finale": [], "titles_id": []}
        return json.dumps(movie_dict)
